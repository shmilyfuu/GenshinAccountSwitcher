...
namespace
{
    ...
    hstring H(std::wstring const& value)
    {
...
namespace winrt::GenshinAccountSwitcher::implementation
{
    ...
    {
        if (m_initialized) RefreshUi(true);
    }
    ...
    void MainWindow::RefreshUi(bool rebuildAccounts)
    {
        ...
        try
        {
            ...
            {
                CurrentAccountText().Text(m_currentState.hasAdl ? L"未保存账号" : L"未检测到登录态");
            }
            ...
            UpdateButtonStates();
            if (m_currentState.matchKind == gas::CurrentMatchKind::UniqueUidCredentialChanged)
                SetStatus(L"已按 UID 识别为已保存账号；当前凭据与保存快照不同。");
            else
                SetStatus(L"状态已刷新。");
        }
        catch (...)
    ...
    MainWindow::AccountVisual MainWindow::BuildAccountVisual(int index)
    {
        ...
        AccountVisual visual;
        visual.item = ListViewItem();
        visual.item.HorizontalContentAlignment(HorizontalAlignment::Stretch);
        visual.item.Padding(MakeThickness(0, 0, 0, 0));
        visual.item.Margin(MakeThickness(0, 0, 0, 6));
        visual.item.MinHeight(50);

        visual.card = Border();
        visual.card.CornerRadius(CornerRadius{ 8 });
        visual.card.BorderThickness(Thickness{ 1 });
        visual.card.Padding(MakeThickness(0, 0, 12, 0));
        ...
        visual.accent.CornerRadius(CornerRadius{ 2 });
        visual.accent.Margin(MakeThickness(0, 7, 0, 7));
        row.Children().Append(visual.accent);
        ...
        name.Margin(MakeThickness(12, 0, 8, 0));
        ...
        visual.badge = Border();
        visual.badge.CornerRadius(CornerRadius{ 9 });
        visual.badge.Padding(MakeThickness(9, 3, 9, 3));
        visual.badge.HorizontalAlignment(HorizontalAlignment::Left);
        visual.badge.VerticalAlignment(VerticalAlignment::Center);
        visual.badge.Visibility(Visibility::Collapsed);
        visual.badgeText = TextBlock();
        visual.badgeText.FontSize(11);
        visual.badgeText.FontWeight(Windows::UI::Text::FontWeights::SemiBold());
        visual.badge.Child(visual.badgeText);
        Grid::SetColumn(visual.badge, 3);
        row.Children().Append(visual.badge);

        visual.card.Child(row);
        visual.item.Content(visual.card);
        return visual;
    }
    ...
    void MainWindow::UpdateAccountVisuals()
    {
        for (int i = 0; i < static_cast<int>(m_visuals.size()); ++i)
        {
            ...
            bool current = i == m_currentState.accountIndex &&
                (m_currentState.matchKind == gas::CurrentMatchKind::ExactCredential || m_currentState.matchKind == gas::CurrentMatchKind::UniqueUidCredentialChanged);
            bool changed = current && m_currentState.matchKind == gas::CurrentMatchKind::UniqueUidCredentialChanged;

            visual.card.Background(selected ? MakeBrush(0xFF, 0x36, 0x36, 0x36) : MakeBrush(0xFF, 0x29, 0x29, 0x29));
            visual.card.BorderBrush(selected ? MakeBrush(0xFF, 0x68, 0x68, 0x68) : MakeBrush(0xFF, 0x3A, 0x3A, 0x3A));
            visual.accent.Background(selected ? MakeBrush(0xFF, 0x60, 0xCD, 0xFF) : MakeBrush(0x00, 0, 0, 0));

            if (current)
            {
                visual.badge.Visibility(Visibility::Visible);
                visual.badge.Background(changed ? MakeBrush(0xFF, 0x4C, 0x3F, 0x20) : MakeBrush(0xFF, 0x1F, 0x45, 0x57));
                visual.badgeText.Text(changed ? L"当前 · 待更新" : L"当前账号");
            }
            else if (selected)
            {
                visual.badge.Visibility(Visibility::Visible);
                visual.badge.Background(MakeBrush(0xFF, 0x2A, 0x45, 0x58));
                visual.badgeText.Text(L"已选中");
            }
            else
            {
                visual.badge.Visibility(Visibility::Collapsed);
    ...
    void MainWindow::UpdateButtonStates()
    {
        ...
        SwitchButton().IsEnabled(selected && writable);
        SwitchLaunchButton().IsEnabled(selected && writable);
        UpdateButton().IsEnabled(selected && writable);
        RenameButton().IsEnabled(selected);
        DeleteButton().IsEnabled(selected);
        AddButton().IsEnabled(writable);
        RestoreButton().IsEnabled(writable && m_core.RecoveryExists());
    }
    ...
    fire_and_forget MainWindow::AddCurrentAsync()
    {
        ...
        try
        {
            ...
            if (probe.uidMatches.size() == 1)
            {
                ...
                TextBlock text;
                text.Text(H(L"UID " + uid + L" 已保存为“" + existing.name + L"”，当前登录凭据与保存快照不同。\n\n可以更新现有记录，也可以另存一条新记录。"));
            ...
            {
                if (!(co_await ConfirmAsync(L"存在多条相同 UID 记录", L"当前 UID 在已保存账号中有多条记录。继续后会把当前登录态另存为新记录。", L"继续另存"))) co_return;
            }
        ...
        {
            co_await ShowMessageAsync(L"添加失败", L"保存当前账号时发生异常。请确认游戏已经完全退出，并检查程序目录写入权限。");
        }
    }
    ...
    fire_and_forget MainWindow::UpdateSelectedAsync()
    {
        ...
        try
        {
            auto result = m_core.UpdateAccount(m_selectedIndex, m_core.ProbeCurrent().snapshot);
            RefreshUi(true);
            co_await ShowMessageAsync(result.success ? L"更新完成" : L"更新失败", result.message);
        }
        catch (...)
    ...
    fire_and_forget MainWindow::SwitchSelectedAsync(bool launchAfter)
    {
        ...
        if (!result.success)
        {
            co_await ShowMessageAsync(L"切换失败", result.message);
            co_return;
        }
        if (!launchAfter)
        {
            co_await ShowMessageAsync(L"切换完成", L"当前账号：" + account.name + L"\nUID：" + account.uid);
            co_return;
        }
        auto state = m_core.DetectCurrentState();
        if (state.accountIndex != targetIndex || state.matchKind != gas::CurrentMatchKind::ExactCredential)
        {
            co_await ShowMessageAsync(L"状态复核失败", L"注册表写入完成后没有精确匹配目标账号，因此没有启动游戏。");
            co_return;
        }
        auto launch = m_core.LaunchGame();
        if (!launch.success) co_await ShowMessageAsync(L"启动失败", launch.message);
        else SetStatus(L"已切换并启动：" + account.name);
    }

    fire_and_forget MainWindow::RestoreAsync()
    {
        auto lifetime = get_strong();
        if (!(co_await ConfirmAsync(L"恢复上一次状态", L"确定恢复最近一次账号切换前保存的注册表快照吗？", L"恢复"))) co_return;
    ...
    void MainWindow::OnSelectGamePathClick(IInspectable const&, RoutedEventArgs const&)
    {
        ...
        RefreshUi(false);
        ShowMessage(result.success ? L"路径已保存" : L"路径无效", result.message);
    }
    ...
    IAsyncOperation<hstring> MainWindow::PromptTextAsync(std::wstring const& title, std::wstring const& message, std::wstring const& initial, std::wstring const& placeholder)
    {
        StackPanel panel;
        panel.Spacing(10);
    ...
    IAsyncOperation<bool> MainWindow::ConfirmAsync(std::wstring const& title, std::wstring const& message, std::wstring const& primary)
    {
        ContentDialog dialog;
        dialog.XamlRoot(RootGrid().XamlRoot());
        dialog.Title(box_value(H(title)));
    ...
    IAsyncAction MainWindow::ShowMessageAsync(std::wstring const& title, std::wstring const& message)
    {
        ContentDialog dialog;
        dialog.XamlRoot(RootGrid().XamlRoot());
        dialog.Title(box_value(H(title)));
